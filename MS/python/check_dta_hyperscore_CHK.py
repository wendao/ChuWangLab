...
for hit in ms.load_dta(dta):
  ...
  if star in realseq:
    ...
    while np > -1:
      ...
      allseq = ms.scan_hyperscore( rawseq, diffmass, mods, star, sp )
      #print calc_mass, hit.calc_mass, "diff=", fabs(calc_mass-hit.calc_mass)

      #filter impossible sites 
      topseq = []
      for seq, hs in allseq:
        ndx = seq.find(star)
        if seq[ndx-1] in possible_AA:
          topseq.append((seq,hs))
      ...
      for result in results:
        ...
        if w>=max_w: best.append(result[0])
